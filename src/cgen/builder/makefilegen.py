    ...
    def _detect_stc_path(self) -> Optional[str]:
        ...
        try:
            ...
            possible_paths = [
                ...
                Path.cwd() / "src" / "cgen" / "ext" / "stc" / "include",
...
class MakefileGenerator:
    ...
    def generate_header(self) -> "MakefileGenerator":
        ...
        self.comment("=" * 60)
        self.comment(f"CGen Project Makefile: {self.name}")
        if self.use_stc:
            self.comment("Enhanced with STC (Smart Template Containers) support")
        self.comment("Generated by CGen Makefile Generator")
        self.comment("=" * 60)
        self.blank_line()
        return self

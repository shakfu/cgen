 ...
 * Note: This function uses vec_cstr which must be declared by the generated code
 */
// cgen_string_list_t cgen_cstr_split(const cstr* s, const char* delimiter); // Disabled to avoid type conflicts

/**
 * Python str.join() semantics
 * Joins vector of cstr with delimiter
 */
// cstr cgen_cstr_join(const char* delimiter, const vec_cstr* strings); // Disabled to avoid type conflicts
 ...
 * Note: STC provides cstr_str() inline function, so we don't redeclare it here
 * The inline function from STC will be used directly
 */
// const char* cstr_str(const void* cstr_ptr); // Disabled - use STC's inline cstr_str instead

/**
 * Bridge function to create vec_cstr for testing string split
 * Note: This function declaration will be generated by the macro below
 * where vec_cstr is properly defined by STC templates
 */
// vec_cstr cgen_create_test_vec_cstr(const char* str, const char* delimiter); // Disabled to avoid type conflicts
...
#define CGEN_IMPLEMENT_STRING_SPLIT_HELPERS() \
    vec_cstr cgen_create_test_vec_cstr(const char* str, const char* delimiter) { \

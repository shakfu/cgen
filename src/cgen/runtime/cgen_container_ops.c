/**
 * CGen Runtime Library - STC Container Helper Operations Implementation
 */
...
struct cgen_container_registry {
...
struct cstr* cgen_cstr_from(const char* str) {
    ...
    CGEN_SET_ERROR(CGEN_ERROR_RUNTIME, "STC cstr integration not yet available");
    return NULL;
}

const char* cgen_cstr_to_cstring(struct cstr* cstr) {
    if (!cstr) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "cstr is NULL");
        return NULL;
    }
...
void* cgen_vec_at_safe(void* vec_ptr, size_t index, size_t element_size,
    ...
    if (!vec_ptr || !size_func) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid vector or size function");
        return NULL;
    }

    size_t size = size_func(vec_ptr);
    if (!cgen_vec_bounds_check(index, size, container_name)) {
        return NULL;
    }
...
void* cgen_hmap_get_safe(void* hmap_ptr, const void* key,
    ...
    if (!hmap_ptr || !key || !get_func) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid hashmap or get function");
        return NULL;
    }

    void* result = get_func(hmap_ptr, key);
    if (!result) {
        CGEN_SET_ERROR_FMT(CGEN_ERROR_KEY, "Key '%s' not found in hashmap",
...
void cgen_vec_enumerate(void* vec_ptr, size_t element_size,
    ...
    if (!vec_ptr || !size_func || !at_func || !callback) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid parameters for vector enumeration");
        return;
    }
...
void cgen_hmap_items(void* hmap_ptr,
    ...
    if (!hmap_ptr || !iter_func || !callback) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid parameters for hashmap iteration");
        return;
    }
...
struct vec_cstr* cgen_string_array_to_vec_cstr(cgen_string_array_t* str_array) {
    ...
    CGEN_SET_ERROR(CGEN_ERROR_RUNTIME, "STC vec_cstr integration not yet available");
    return NULL;
}

cgen_string_array_t* cgen_vec_cstr_to_string_array(struct vec_cstr* vec) {
    if (!vec) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Vector is NULL");
        return NULL;
    }
...
cgen_container_registry_t* cgen_container_registry_new(void) {
    cgen_container_registry_t* registry = malloc(sizeof(cgen_container_registry_t));
    if (!registry) {
        CGEN_SET_ERROR(CGEN_ERROR_MEMORY, "Failed to allocate container registry");
        return NULL;
    }
...
void cgen_container_registry_free(cgen_container_registry_t* registry) {
    ...
    cgen_cleanup_containers(registry);
    free(registry);
}
...
cgen_error_t cgen_register_container(cgen_container_registry_t* registry,
    ...
    if (!registry || !container || !cleanup_func) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid parameters for container registration");
        return CGEN_ERROR_VALUE;
    }

    container_entry_t* entry = malloc(sizeof(container_entry_t));
    if (!entry) {
        CGEN_SET_ERROR(CGEN_ERROR_MEMORY, "Failed to allocate container entry");
        return CGEN_ERROR_MEMORY;
    }

    entry->container = container;
    entry->cleanup_func = cleanup_func;
    entry->name = name ? cgen_strdup(name) : NULL;
...
int cgen_bool_container(void* container, size_t (*size_func)(void*)) {
    return cgen_len(container, size_func) > 0;
}
...
int cgen_in_hmap(void* hmap_ptr, const void* key,
                int (*contains_func)(void*, const void*)) {
    return cgen_hmap_contains_key(hmap_ptr, key, contains_func);
}
...
char* cgen_vec_repr(void* vec_ptr,
    ...
    if (!vec_ptr || !size_func || !at_func || !element_repr) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid parameters for vector representation");
        return NULL;
    }
    ...
    cgen_string_array_t* parts = cgen_string_array_new();
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (element) {
            ...
            if (elem_repr) {
                if (cgen_string_array_add(parts, elem_repr) != CGEN_OK) {
                    free(elem_repr);
    ...
    if (!result) {
        free(joined);
        CGEN_SET_ERROR(CGEN_ERROR_MEMORY, "Failed to allocate memory for vector representation");
        return NULL;
    }
...
char* cgen_hmap_repr(void* hmap_ptr, char* (*repr_func)(void*)) {
    if (!hmap_ptr || !repr_func) {
        CGEN_SET_ERROR(CGEN_ERROR_VALUE, "Invalid parameters for hashmap representation");
        return NULL;
    }

    // This would be implemented with actual STC hashmap iteration when available
    return cgen_strdup("{}"); // Simplified representation
}
...
class NestedContainerManager:
    ...
    def _generate_template_definition(self, type_name: str, type_info: NestedTypeInfo,
                                    template_manager) -> List[str]:
        ...
        if stc_type == 'vec':
            ...
            instance_name = template_manager.register_container_usage(
                type_name, 'vec', [c_element_type], 'stc/vec.h'
            )
            ...
            # Register with template manager
            instance_name = template_manager.register_container_usage(
                type_name, 'hset', [c_element_type], 'stc/hset.h'
            )
    ...
    def is_nested_container_type(self, type_str: str) -> bool:
        ...
        return max_depth > 1 or (max_depth == 1 and any(
            container in type_str for container in ['List[', 'Dict[', 'Set[', 'list[', 'dict[', 'set[']
        ))

...
class CGenWriter(Formatter):
    ...
    def _write_function_declaration(self, elem: core.Function) -> None:
        """Writes function declaration or definition."""
        if elem.extern:
            self._write("extern ")
        if elem.static:
            self._write("static ")
        ...
        # CGen extension: handle function body if present
        if hasattr(elem, 'body') and elem.body is not None:
            # This is a function definition, not just a declaration
            self._write(" ")
            self._write_block(elem.body)
        else:
            # Just a declaration, add semicolon
            pass  # Semicolon will be added by Declaration writer

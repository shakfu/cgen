...
class Function(Element):
    """Function declaration with optional body support for CGen."""

    def __init__(
        self,
        name: str,
        return_type: Union[str, DataType, None] = None,
        static: bool = False,
        ...
        # CGen extension: support function body
    ...
    def set_body(self, body: "Block") -> "Function":
        """Sets the function body. CGen extension for full function definitions."""
        if not isinstance(body, Block):
            raise TypeError("body must be a Block object")
        self.body = body
        return self

    def add_statement(self, statement: Union[str, Element]) -> "Function":
        """Adds a statement to the function body. Creates body if it doesn't exist."""
        if self.body is None:
            self.body = Block()

        if isinstance(statement, str):
            self.body.append(Statement(statement))
        elif isinstance(statement, Element):
            self.body.append(statement)
        else:
            raise TypeError("statement must be str or Element")
        return self

    def is_declaration_only(self) -> bool:
        """Returns True if this is just a declaration (no body)."""
        return self.body is None

    def is_definition(self) -> bool:
        """Returns True if this is a full definition (has body)."""

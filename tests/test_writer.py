"""Pytest-style tests for writer class."""

import cgen.generator as cfile
import cgen.generator.core as core
...
class TestPreprocessor:
    ...
    def test_ifndef(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == "#ifndef IDENTIFIER"
...
class TestVariableDeclaration:
    ...
    def test_type_is_const_int_ptr__var_is_const_ptr__left_align(self):
        ...
        writer = cfile.Writer(
            cfile.StyleOptions(pointer_alignment=style.Alignment.LEFT))
        output = writer.write_str_elem(element)
        assert output == "const int** const value"
...
class TestFunctionDeclaration:
    ...
    def test_int_func_with_int_arg_and_char_double_ptr_arg(self):
        ...
        assert output == "int main(int argc, char** argv)"
...
class TestStatement:
    ...
    def test_struct_ref_variable_declaration(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == "struct record_t a;"
    ...
    def test_struct_with_int_initializer(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == "struct record_t a = {0, 0};"
    ...
    def test_const_str_var_with_str_initializer(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == 'const char* s = "Static Text";'
...
class TestFunctionReturn:
    ...
    def test_float(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == "return 0.0"

    def test_bool(self):
        element = core.FunctionReturn(False)
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == "return false"

    def test_str(self):
        element = core.FunctionReturn("retval")
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element)
        assert output == "return retval"
...
class TestTypedef:
    ...
    def test_type_is_int(self):
        ...
        assert output == "typedef int MyInt"

    def test_type_is_const_int(self):
        datatype = core.TypeDef("MyConstInt", "int", const=True)
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(core.Declaration(datatype))
        assert output == "typedef const int MyConstInt"

    def test_type_is_int_ptr(self):
        datatype = core.TypeDef("MyIntPtr", "int", pointer=True)
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(core.Declaration(datatype))
        assert output == "typedef int* MyIntPtr"
    ...
    def test_type_is_int_ptr__typedef_is_ptr_left_align(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(core.Declaration(datatype))
        assert output == "typedef int** MyIntPtrPtr"
...
class TestLine:
    ...
    def test_str_line(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element, trim_end=False)
        assert output == "Any code expression\n"
    ...
    def test_statement_with_comment(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(element, trim_end=False)
        assert output == "int value; //Comment\n"
        ...
        expected = """struct MyStruct
{
        ...
        assert expected == writer.write_str_elem(core.Declaration(struct))
        ...
        expected = """typedef struct os_alarm_cfg_tag
{
    ...
    uint32_t periodMs;
        ...
        assert expected == writer.write_str_elem(statement)


class TestStructReference:

    def test_struct_variable(self):
        struct = core.Variable("time", core.Struct("timespec"))
        writer = cfile.Writer(cfile.StyleOptions())
        assert "struct timespec time" == writer.write_str_elem(core.Declaration(struct))

    def test_struct_parameter(self):
        func = core.Function("process_time", "void", params=[core.Variable("time",
                                                             core.Struct("timespec"))])
        writer = cfile.Writer(cfile.StyleOptions())
        assert "void process_time(struct timespec time)" == writer.write_str_elem(core.Declaration(func))
    ...
    def test_struct_return_and_struct_parameter(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        output = writer.write_str_elem(core.Declaration(func))
        assert "static struct timespec timespec_add(struct timespec time1, struct timespec time2)" == output
    ...
    def test_struct_forward_declaration(self):
        ...
        writer = cfile.Writer(cfile.StyleOptions())
        assert "struct os_task_tag;" == writer.write_str_elem(struct)

...
def generate_c_code_with_integrated_generator(analysis_results):
    ...
    main_func.add_statement(f'printf("• Compile-time optimization: {speedup:.2f}x speedup\\n")')

    if vector_result and vector_result.vectorizable_loops > 0:
        main_func.add_statement('printf("• Vectorization opportunities identified\\n")')

    if bounds_result and bounds_result.confidence > 0.8:
        main_func.add_statement('printf("• Memory safety verification completed\\n")')
...
def main():
    ...
    print("=" * 60)
    print("Python Analysis → C Generation (with function bodies) → Compilation → Execution")
    print("=" * 60)
    ...
    if success:
        ...
        print(f"Python → Intelligence Analysis → C Generation (with bodies) → Compilation → Execution ✅")
    else:
